...
class GaussianBindingModule(nn.Module):
    def __init__(self, feature_dim: int, hidden_dim: int, anchor_dim: int = 10, gate_init_bias: float = -2.2):
        super().__init__()
        ...
        input_dim = anchor_dim + 4 * feature_dim + 2
    ...
    def _build_fused_state(self, anchor_state, rgb_flat, thermal_flat):
    ...
    def forward(self, anchor_state, rgb_features, thermal_features, thermal_context_features=None):
        ...
        fused_state = self._build_fused_state(anchor_state, rgb_flat, thermal_context_flat)
